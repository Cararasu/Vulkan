...
RendResult VBuffer::init() {
	...
	v_logger.log<LogLevel::Debug> ( "Create Buffer of size %" PRId64 " with usage %s with buffer 0x%" PRIx64, size, to_string ( usage ).c_str(), reinterpret_cast<u64> ( static_cast<VkBuffer> ( buffer ) ) );
	...
	if ( memory.memory ) {
		return RendResult::Fail;
	}
	return RendResult::Success;
}
RendResult VBuffer::map_mem() {
	if ( !memory.memory ) {
		return RendResult::Uninitialized;
	}
	...
	return mapped_ptr ? RendResult::Success : RendResult::Fail;
}
