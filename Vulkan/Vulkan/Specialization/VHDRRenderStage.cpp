...
void gen_hdr_pipeline ( VInstance* v_instance, PipelineStruct* p_struct, u32 pipeline_index, Viewport<f32> viewport, vk::RenderPass renderpass ) {
	if ( !p_struct->pipelines[pipeline_index] ) {
		...
		VShaderModule* fmod = v_instance->m_resource_manager->v_get_shader ( StringReference ( "hdr_shader" ) );
...
VHDRRenderStage::VHDRRenderStage ( VInstance* instance ) :
	VRenderStage ( RenderStageType::eRendering ),
	v_instance ( instance ),
	hdr_pipeline ( fullscreen_modelbase_id, single_instance_base_id, { postproc_context_base_id }, {}, 1 ),
	renderpasses ( 1 ),
	subpass_inputs ( 1 ) {
...
VHDRRenderStage::~VHDRRenderStage() {

	v_destroy_framebuffers();
	v_destroy_pipeline_layouts();
}
void VHDRRenderStage::v_destroy_pipeline_layouts() {
	v_destroy_renderpasses();
	v_destroy_pipelines();

	destroy_pipeline_layout ( v_instance, &hdr_pipeline );
}
void VHDRRenderStage::v_destroy_pipelines() {
	v_destroy_framebuffers();
	destroy_pipeline ( v_instance, &hdr_pipeline );
}
void VHDRRenderStage::v_destroy_renderpasses() {
...
void VHDRRenderStage::v_destroy_framebuffers() {
	for ( RenderPassWrapper& wrap : renderpasses ) {
		if ( wrap.framebuffer ) {
...
void VHDRRenderStage::set_renderimage ( u32 index, Image* image, u32 miplayer, u32 arraylayer ) {
...
void VHDRRenderStage::v_rebuild_pipelines() {
	PushConstUsed push = {0, 4};
	gen_pipeline_layout ( v_instance, &subpass_inputs[0], &hdr_pipeline, &push );
	...
	for ( int i = 0; i < renderpasses.size; i++ ) {
		...
		if ( !hdr_pipeline.pipelines[i] ) {
			...
			gen_hdr_pipeline ( v_instance, &hdr_pipeline, i, t_viewport, wrap.renderpass );
		}
	}
	last_frame_index_pipeline_built = v_instance->frame_index;
}
void VHDRRenderStage::v_dispatch ( vk::CommandBuffer buffer, u32 index ) {
	v_check_rebuild();
	v_rebuild_pipelines();
	...
	for ( int i = 0; i < renderpasses.size; i++ ) {
		...
		buffer.pushConstants ( hdr_pipeline.pipeline_layout, vk::ShaderStageFlagBits::eAllGraphics, 0, 4, &i );
		render_pipeline ( v_instance, v_instancegroup, v_contextgroup, &hdr_pipeline, &subpass_inputs[0], buffer, i );
