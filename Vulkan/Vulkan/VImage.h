...
struct VWindowImage : public VBaseImage {
	...
	void new_window_images ( DynArray<vk::Image>& images,
	                         vk::Extent3D extent,
	                         vk::Format format );

	virtual void transition_image_layout ( vk::ImageLayout newLayout,
	                                       Range<u32> mip_range,
	                                       Range<u32> array_range,
	                                       vk::CommandBuffer commandBuffer );
	virtual void generate_mipmaps ( Range<u32> mip_range,
	                                Range<u32> array_range,
...
struct VImageWrapper : public VBaseImage {
	...
	VImageWrapper ( VInstance* instance,
	                ...
	                vk::MemoryPropertyFlags recommended = vk::MemoryPropertyFlags() );
	virtual ~VImageWrapper();

	void init();
	void destroy();

	void rebuild_image ( vk::Extent3D extent );
